...
class ISOBloc extends Cubit<ISOApiState> {
  ...
  Future<ISOApiState> getMyAndTeamIbtakerIdeas(
      {required Map<String, dynamic> requestParams,
      bool emitResponse = false}) async {
    if (emitResponse) {
      emit(OnISOApiLoading());
    }
    final result = await isoUseCase.getMyAndTeamIbtakerIdeas(
  ...
  Future<ISOApiState> getIbtakerUsers(
      {required Map<String, dynamic> requestParams,
      bool emitResponse = false}) async {
    if (emitResponse) {
      emit(OnISOApiLoading());
    ...
    final result =
        await isoUseCase.getIbtakerUsers(requestParams: requestParams);
    final apiState =
        result.fold((l) => OnISOApiError(message: l.errorMessage), (r) {
      return OnISOApiResponse(response: r);
    });
